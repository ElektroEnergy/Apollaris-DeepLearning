...
def pvl_absoluteairmass(AMrelative = None,pressure = None): 
...
#   Solar Energy, vol. 51, pp. 121-138, 1993.
    ...
    p = inputParser
    p.addRequired('AMrelative',lambda x = None: np.all(np.logical_or(isnumeric(x),np.isnan(x))))
    p.addRequired('pressure',lambda x = None: np.all(np.logical_and(isnumeric(x),x) >= 0))
    p.parse(AMrelative,pressure)
    AMa = np.multiply(AMrelative,pressure) / 101325
    return AMa