...
for i in np.arange(1,12+1).reshape(-1):
    ...
    for j in np.arange(np.arange(1,len(DecHours(,,1))+1)):
        ...
        GMSTi[j,i] = np.mod(GMST0(i) + 360 * (1.0027379093 * UnivHr(j,i) / 24),360)
        LocAST[j,i] = np.mod((360 + GMSTi(j,i) - Longitude),360)
        EpochDate[j,i] = Ezero(i) + UnivHr(j,i) / 24
        T1[j,i] = EpochDate(j,i) / 36525
        ObliquityR[j,i] = DegtoRad * (23.452294 - 0.0130125 * T1(j,i) - 1.64e-06 * T1(j,i) ** 2 + 5.03e-07 * T1(j,i) ** 3)
        MlPerigee[j,i] = 281.22083 + 4.70684e-05 * EpochDate(j,i) + 0.000453 * T1(j,i) ** 2 + 3e-06 * T1(j,i) ** 3
        MeanAnom[j,i] = np.mod((358.47583 + 0.985600267 * EpochDate(j,i) - 0.00015 * T1(j,i) ** 2 - 3e-06 * T1(j,i) ** 3),360)
        Eccen[i] = 0.01675104 - 4.18e-05 * T1(j,i) - 1.26e-07 * T1(j,i) ** 2
        EccenAnom[j,i] = MeanAnom(j,i)
        E[j,i] = 0
        while np.amax(np.abs(EccenAnom(j,i) - E(j,i))) > 0.001:

            E[j,i] = EccenAnom(j,i)
            EccenAnom[j,i] = MeanAnom(j,i) + np.multiply(np.multiply(RadtoDeg,Eccen(i)),np.sin(np.multiply(DegtoRad,E(j,i))))

        TrueAnom[j,i] = 2 * np.mod(RadtoDeg * atan2(np.multiply(((1 + Eccen(i)) / (1 - Eccen(i))) ** 0.5,np.tan(DegtoRad * EccenAnom(j,i) / 2)),1),360)
        EcLon[j,i] = np.mod(MlPerigee(j,i) + TrueAnom(j,i),360) - Abber
        EcLonR[j,i] = DegtoRad * EcLon(j,i)
        DecR[j,i] = np.arcsin(np.multiply(np.sin(ObliquityR(j,i)),np.sin(EcLonR(j,i))))
        Dec[j,i] = RadtoDeg * DecR(j,i)
        RtAscen[j,i] = RadtoDeg * atan2(np.multiply(np.cos(ObliquityR(j,i)),(np.sin(EcLonR(j,i)))),np.cos(EcLonR(j,i)))
        HrAngle[j,i] = LocAST(j,i) - RtAscen(j,i)
        HrAngleR[j,i] = np.multiply(DegtoRad,HrAngle(j,i))
...
# HrAngleR(i) = DegtoRad .* HrAngle(i)
        ...
        SunAz[j,i] = np.multiply(RadtoDeg,atan2(- 1 * np.sin(HrAngleR(j,i)),np.multiply(np.cos(LatR),np.tan(DecR(j,i))) - np.multiply(np.sin(LatR),np.cos(HrAngleR(j,i)))))
        SunAz[j,i] = SunAz(j,i) + (SunAz(j,i) < 0) * 360
        SunEl[j,i] = asind(np.multiply(np.multiply(np.cos(LatR),np.cos(DecR(j,i))),np.cos(HrAngleR(j,i))) + np.multiply(np.sin(LatR),np.sin(DecR(j,i))))
        SolarTime[j,i] = (180 + HrAngle(j,i)) / 15
        # Calculate the refraction of the sun until the actual center of the sun is
# 1 degree below the horizon.
        TanEl[j,i] = np.tan(DegtoRad * SunEl(j,i))
        ...
        ApparentSunEl[j,i] = SunEl(j,i) + Refract(j,i)
